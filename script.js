...
function checkPallindromeForAllDateFormats(date) {
  ...
  for (i=0; i<listOfPallindromes.length; i++) {
...
function getNextPallindromeDate(date) {
  ...
  while(1) {
    count += 1;
    if(checkPallindromeForAllDateFormats(nextDate)){
      break
    } else {
...
function clickHandler() {
  ...
  if(bdayStr !== "") {
    ...
    if(isPallindrome) {
      resultRef.innerHTML = "Yay your birthday is a Pallindrome";
    } else {
      var [count, nextDate] = getNextPallindromeDate(date);
      resultRef.innerHTML = `The next Pallindrome date is ${nextDate.day}-${nextDate.month}-${nextDate.year}, you missed it by ${count} days!`; 
    }
  }
}
